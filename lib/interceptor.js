...
function Interceptor(typeOrInstance) {
	...
	self.dispatcher = require("./dispatcher");
	...
	var initProxy = function() {
		assert(self.proxy, "Please make sure you use the 'forMember()', forObject()' or 'forType()' method before initializing a proxy");
		...
		self.proxiedInstance = self.proxy.whenCalled(function(instance, member, args, memberName) {
			...
			try {
				self.emitBeforeEvent(invocation);
				self.dispatcher.dispatch(invocation);
				self.emitAfterEvent(invocation);
			} catch (err) {
