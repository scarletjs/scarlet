...
function Scarlet(pluginArr) {
	...
	var self = this;
	self.plugins = {};
	self.pluginManager = new Plugins();
	...
	self.intercept = function(typeOrInstance, memberName) {
		g.assert(typeOrInstance, "Cannot have null type or instance");
		g.assert(typeof(typeOrInstance.__scarlet) === "undefined", "Type or instance already contains a scarlet interceptor");
		var self = this;
		var _interceptor = new Interceptor(typeOrInstance);
		if (!ext.object.isNull(memberName)) {
		...
		if (!ext.object.isNull(typeOrInstance.prototype))
	...
	self.loadPlugin = function(pluginPath) {
		self.pluginManager.loadPlugin(self, pluginPath);
		return self;
	};
