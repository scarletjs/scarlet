...
function Dispatcher() {
	...
	self.invokeCall = function(invocation, currentCall){
		...
		if (self.isAsynchronous(currentCall.method)) {
			currentCall.method.call(currentCall.thisContext, invocation.proceed, invocation, function(){
				self.invokeCall(invocation, currentCall.next());
			});
		} else {
			currentCall.method.call(currentCall.thisContext, invocation.proceed, invocation);
			self.invokeCall(invocation, currentCall.next());
		}
	};
