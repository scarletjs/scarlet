...
function Scarlet(pluginArr) {
	...
	var dispatchEvent = function(eventName, thisContext, info, method, args, result) {
		self.emit(eventName, {
			info: info,
			args: args,
			result: result,
	...
	self.using = function(callback) {
		...
		interceptor.using(function(info, method, args) {
			dispatchEvent("before", this, info, method, args, null);
			var result = callback.call(this, info, function(_info, _method, _args) {
				var _result = null;
				try {
					_result = method.call(this, _info, _method, _args);
				} catch (err) {
					dispatchEvent("error", this, _info, _method, _args, _result);
				}
				dispatchEvent("done", this, _info, _method, _args, _result)
				return _result;
			}, args);
			dispatchEvent("after", this, info, method, args, result);
			return result;
		});
		return self;
	};
