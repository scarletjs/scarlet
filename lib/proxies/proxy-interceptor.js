...
function ProxyInterceptor(typeOrInstance, memberName) {
	...
	var interceptFunction = function(whenCalledCallback, replaceClassCallback) {
		var proxyInfo = new ProxyInfo(self.typeOrInstance,memberName);		
		self.interceptor = new ProxyFunction(proxyInfo, whenCalledCallback);
	...
	var interceptMethod = function(whenCalledCallback, replaceClassCallback) {
		var proxyInfo = new ProxyInfo(self.typeOrInstance,memberName);		
		self.interceptor = new ProxyMethod(proxyInfo, whenCalledCallback);
		logger.info(ProxyInterceptor, "intercept", "Using a Proxy Function", [self.interceptor]);
	};

	var interceptProperty = function(whenCalledCallback, replaceClassCallback) {
		var proxyInfo = new ProxyInfo(self.typeOrInstance,memberName);
		self.interceptor = new ProxyProperty(proxyInfo, whenCalledCallback);
		logger.info(ProxyInterceptor, "intercept", "Using a Proxy Function", [self.interceptor]);
	};
	...
	var setInterceptorType = function(whenCalledCallback, replaceClassCallback){
		if(memberName){
			if(typeof self.typeOrInstance[memberName] === 'function')
				return interceptMethod(whenCalledCallback,replaceClassCallback);

			return interceptProperty(whenCalledCallback,replaceClassCallback);
		}
